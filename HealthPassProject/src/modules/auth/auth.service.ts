...
export const authService = {
  async sendOtp(mobile: string, isSignup: boolean = false) {
    // Check if user exists
    const existingUser = await authRepository.findUserByMobile(mobile);
    if (isSignup && existingUser && existingUser.isProfileComplete) {
      throw new AppError('Number already exists', 409);
    }
    if (!isSignup && !existingUser) {
      throw new AppError('Number not found. Please sign up first.', 404);
    }
  ...
  async verifyOtp(mobile: string, otp: string) {
    // Static OTP for testing - Admin user
    if (mobile === '9199999998' && otp === '123456') {
      let user = await authRepository.findUserByMobile(mobile);
      if (!user) {
        user = await authRepository.createUser(mobile, 'ADMIN');
    ...
    // Static OTP for testing - Regular user
    if (mobile === '9199999999' && otp === '123456') {
      let user = await authRepository.findUserByMobile(mobile);
      if (!user) {
    ...
    console.log(`[DEBUG] Verifying OTP for ${mobile}. Entry in DB:`, otpRecord ? 'Found' : 'Not Found');
    ...
    console.log(`[DEBUG] OTP match result for ${mobile}:`, isMatch);
